// Single source of truth for the reservable catalog.
//
...
//
// Every entry becomes one Resource document. Rooms have quantity 1 and a floor;
// equipment carries no floor and its quantity is decided per-request by the
// borrower, so the stored quantity is left at 1 and unused for stock checks.

export const DEPARTMENTS = ['BMO', 'ITRO'];
...
export const resources = DEPARTMENTS.flatMap((department) => {
  const rooms = Object.entries(ROOMS_BY_FLOOR[department] || {}).flatMap(([floor, names]) =>
    names.map((name) => ({ department, kind: 'room', floor, name, quantity: 1, active: true }))
  );
  const equipment = (EQUIPMENT[department] || []).map((name) => ({
    department,
    kind: 'equipment',
    floor: '',
    name,
